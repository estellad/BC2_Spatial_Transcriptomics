...
Read in Chromium as Seurat object. The Chromium annotation is derived by 10X using marker genes and cell atlas reference. For the sake of time, we could directly load in the saved raw Chromium data in the next chunk. 
```{r, eval=FALSE}
chrom_path <- paste0(root_path, "Chromium/")
Counts <- Seurat::Read10X_h5(paste0(chrom_path, "filtered_feature_bc_matrix.h5")) # 18082 30365  gene * cells barcodes
# dim(Counts)

MT <- read.csv(paste0(chrom_path, "chrom_cell_annotations.csv")) # 27472 cell barcodes
MT <- column_to_rownames(MT, "Barcode")
# dim(MT)

overlap_cell_barcode <- intersect(rownames(MT), colnames(Counts)) # 27460 cell barcodes
...
chrom <- CreateSeuratObject(counts = Counts_,
                            meta.data = MT_) # 18082 27460
# dim(chrom)

# saveRDS(chrom, "./intermediate_data/chrom_raw.rds")
rm(Counts_, MT_, Counts, MT); gc()
```


QC Chromium with R package `SeuratQCUtils`, which derives flags inspired by `scuttle::isOutlier()` function from OSCA book. We filter out cells with low library size, high mitochondria percentage, and low abundance genes with mean expression level lower than exp(-5) \approx 0.0067.
```{r}
...
if(any(chrom_mito_drop)){
  p2 <- plot_Hist_High_Mito_Props(chrom)
}
...
Visualize Chromium QC results
```{r, warning=FALSE}
(p2 + p3) + plot_annotation("Chromium QC")
```

Subset to cells that have successfully past QC, and save the Chromium reference that is ready for cell-type deconvolution.
```{r}
# Subset
...
saveRDS(myRCTD, "./intermediate_data/myRCTD.rds")
```
...
```{r}
myRCTD <- readRDS("./intermediate_data/myRCTD.rds")
...
```{r}
aggxe_path <- "~/Downloads/BC2_data/AggXe/intermediate_data/"
...
saveRDS(aggxe, "./intermediate_data/aggxe.rds")
```
...
```{r}
df <- data.frame(vis_per_cell_sum= vis$nCount_Spatial, aggxe_per_cell_sum = aggxe$nCount_Spatial)
ggplot(df, aes(x = vis_per_cell_sum, y = aggxe_per_cell_sum)) +
  geom_point() +
